...
public class RedisServiceImpl implements RedisService {
    ...
    public Map<Object, Double> zReverseRangeWithScore(String key, long start, long end) {
        return Optional.ofNullable(redisTemplate.opsForZSet().reverseRangeWithScores(key, start, end)).map(e->e
                .stream()
                .collect(Collectors.toMap(ZSetOperations.TypedTuple::getValue, ZSetOperations.TypedTuple::getScore))).orElseThrow(()->new RuntimeException("分数排名为空"));
    }
